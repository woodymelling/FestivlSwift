...
public struct StagesIndicatorView: View {
    ...
    var colors: [Color] {
        stageIDs.compactMap { stages[id: $0]}.sorted(by: \.sortIndex).map(\.color)
    }
...
struct StagesIndicatorView_Previews: PreviewProvider {
    static var previews: some View {
        Group {
            StagesIndicatorView(stageIDs: Stage.previewData.map(\.id))
            StagesIndicatorView(stageIDs: [Stage.testData.id])
        }
